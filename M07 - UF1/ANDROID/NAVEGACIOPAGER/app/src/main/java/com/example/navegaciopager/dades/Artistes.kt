...
object Artistes {
    ...
    private val artistes = (1..100).map { InfoArtistes(it) }

    fun obtenDadesDelsArtistes() = artistes

    fun obtenirArtiste(): Artista {
        val artistes = obtenDadesDelsArtistes()
        return artistes.random()
    }

    fun obtenirArtisteById(id: Int): Artista {
        val art = obtenDadesDelsArtistes()
        val artisteEncontrado = art.find { it.id == id }
        return artisteEncontrado ?: obtenirArtiste()
    }
}
